...
// The job window must span the extreme event timestamps. Causal grouping
// orders events by step with buckets appended last, so positional
// first/last would invert started_at/completed_at whenever an early event
// (e.g. a pre-daemon block) flattens after the last scaffold step.
func TestPushToApi_JobWindowIsMinMaxNotPositional(t *testing.T) {
	var got map[string]any
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		require.NoError(t, json.NewDecoder(r.Body).Decode(&got))
		w.Header().Set("Content-Type", "application/json")
	...
	// buildCausalPushGroups produces for pre-daemon traffic.
	groups := []StepEvents{
		{Step: GitHubStep{Name: "build"}, Events: []events.AuditEvent{
			{Timestamp: base.Add(30 * time.Second), EventType: events.EventConnectionBlocked, DstIP: "1.1.1.1", DstPort: 443, Protocol: "TCP"},
		}},
		{Step: GitHubStep{Name: bucketPreDaemon}, Events: []events.AuditEvent{
			{Timestamp: base, EventType: events.EventConnectionBlocked, DstIP: "2.2.2.2", DstPort: 443, Protocol: "TCP"},
		}},
	}

	c := &SummaryCmd{ApiUrl: srv.URL, Token: "test-token", JobName: "build"}
	_, err := c.pushToApi(groups, nil)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	assert.Equal(t, base, started.UTC(), "started_at is the earliest event, wherever it sits")
	assert.Equal(t, base.Add(30*time.Second), completed.UTC(), "completed_at is the latest event")
}
