...
- **DNS redirect:** iptables DNAT rules redirect all outbound DNS (port 53) to `127.0.0.1:53`, exempting queries marked with `SO_MARK` (`0xCA12`) — the proxy's upstream client and its startup stub peeks (`network.MarkedDialControl`). The systemd-resolved stub (`127.0.0.53`) is DNAT'd explicitly (#110): a client following a stub resolv.conf otherwise reaches resolved, which re-queries upstream from its *own* socket — laundering the querying process away from the sockdiag step join and serving warm cache hits the proxy never sees. `127.0.0.1` itself (the proxy listen) stays un-DNATed; nss-resolve's D-Bus/varlink lookups emit no client DNS packet and remain out of reach (the attach-time cache flush covers them). Install and teardown both flush dport-53 conntrack entries (ctnetlink): nat verdicts are stamped per flow at its first packet, so pre-install DNS flow state would otherwise keep bypassing the proxy (and post-teardown state would keep DNAT'ing to the dead proxy) until natural expiry
